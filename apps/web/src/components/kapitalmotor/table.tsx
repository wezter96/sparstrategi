...
import { alt1WithdrawAtom } from "@/state/kapitalmotor";
          ...
          <tbody className="tabular-nums">
            {result.rows.map((row) => (
              <tr key={row.year} className="border-b border-border/50">
                ...
                <td className="py-1.5 pr-3 text-right">{fmtKr(row.portfolio)}</td>
                <td className="py-1.5 pr-3 text-right text-emerald-400">{fmtKr(row.af)}</td>
                <td className="py-1.5 pr-3 text-right text-indigo-400">{fmtKr(row.isk)}</td>
                <td className="py-1.5 pr-3 text-right">{fmtKr(row.loan)}</td>
                <td className="py-1.5 pr-3 text-right">{fmtPct(row.ltvOfEquity)}</td>
                <td className="py-1.5 pr-3 text-right text-amber-400">
                  {row.year === 0 ? "–" : `− ${fmtKr(row.interest)}`}
                </td>
                <td className="py-1.5 pr-3 text-right text-emerald-400">
                  {row.year === 0 ? "–" : `+ ${fmtKr(row.deduction)}`}
                </td>
                <td className="py-1.5 pr-3 text-right">
                  {row.year === 0 ? "–" : fmtKr(row.consumption)}
                </td>
                <td className="py-1.5 pr-3 text-right text-emerald-400">
